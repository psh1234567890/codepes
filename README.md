...
- 대회 유형, 참가 자격, 진행 방식, 주최기관, 마감순·최근 확인순 필터
- 자주 보는 주최기관을 브라우저에 저장하고 해당 기관의 대회만 보기
- 검색·필터 결과를 월간 캘린더와 날짜별 일정으로 보기
- 브라우저 `localStorage`에 저장되는 관심 대회
- 저장한 대회만 모아 보기
- 전체 일정, 저장한 일정, 개별 일정을 표준 `.ics` 파일로 내보내기
- 24시간·7일·30일 안에 신청 마감 또는 시작하는 대회만 보기
- 캘린더 알림을 없음·1시간·1일·3일 전으로 선택하고 실제 대회 기간도 함께 내보내기
- 관심 대회·주최기관 목록을 JSON 파일로 백업하고 다른 브라우저에서 기존 목록에 합치기
- 선택한 대회의 공유 링크 생성
- 공식 대회 페이지로 바로 이동
- 잘못된 정보와 신규 대회를 공개 GitHub 이슈로 제보
- GitHub의 최신 대회 JSON을 우선 사용하고, 네트워크 또는 검증 실패 시 배포 번들 데이터로 자동 복구

CodePes에는 계정, 로그인, 이메일 구독, 자체 데이터베이스가 없습니다. 관심 목록은 현재 브라우저에 저장됩니다. 다른 기기로 옮기려면 직접 JSON 백업 파일을 내려받아 가져와야 하며 자동 동기화는 제공하지 않습니다. 캘린더 알림은 파일을 가져온 캘린더 앱이 담당하며, 앱마다 알림 지원이 다를 수 있습니다. 내려받은 일정은 자동 갱신되지 않습니다.
...
Devpost는 마감 임박순 최대 80개의 공개 해커톤 목록, 공식 일정, 공식 규정을 함께 확인합니다. 온라인 진행이면서 규정에 대한민국·전 세계·모든 국가 참가 가능 또는 제재 국가 외 참가 가능이 명시된 대회만 게시합니다. AI·머신러닝·LLM 주제는 `AI·데이터`로 분류하면서 `해커톤` 필터에서도 함께 찾을 수 있습니다.

CTFtime에서는 향후 120일 안에 열리며 `Online`과 `Open`이 모두 명시된 보안 CTF만 가까운 일정순으로 최대 30개 가져옵니다. itch.io는 가까운 예정 게임잼의 상세 규정까지 읽고 `누구나 참가 가능`과 온라인 제출 일정이 모두 확인된 항목만 게시합니다.
...
DACON은 공식 홈에서 최대 20개의 대회를 발견하고 일정·개요·규칙을 함께 읽습니다. 대한민국 국민 누구나 참가할 수 있는 온라인 코드 제출 대회 중 신청 마감 시각이 정확히 공개된 항목을 6시간마다 자동 수집합니다. DAKER와 자동 파서가 지원하지 않는 대회는 공식 페이지를 사람이 확인한 뒤 `data/manual-contests.json`에 반영합니다. 동기화 작업은 만료된 항목을 제외하고 URL과 제목·날짜를 기준으로 중복을 제거합니다.
...
검색엔진용 사이트맵과 대회 상세 메타데이터도 Worker에서 최신 검증 JSON을 사용합니다. 데이터만 5분 동안 캐시하며, 원격 확인에 실패하면 마지막 정상 데이터 또는 배포 데이터를 사용합니다.
