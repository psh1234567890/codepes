...
Devpost는 공개 해커톤 목록과 각 해커톤의 공식 일정 페이지를 함께 확인해 정확한 제출 시작·마감 시각을 가져옵니다. 초대 전용 해커톤은 제외하며 연령, 거주지, 팀 구성 같은 조건은 `대회별 확인`으로 표시합니다.

CTFtime에서는 향후 120일 안에 열리는 보안 CTF를 가까운 일정순으로 최대 30개 가져옵니다. itch.io에서는 참가자가 많은 Upcoming Game Jams 목록 중 기간과 시작 시각이 정상적인 항목을 최대 20개 반영합니다. CTF 시작 시각과 게임잼 제출 종료 시각의 의미가 다르므로 화면에서도 일정 기준을 구분합니다.
