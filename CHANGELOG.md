...
- DACON 공식 목록·일정·참가 규칙 기반 국내 온라인 AI 대회 자동 발견과 6시간 주기 갱신
- 마감·시작까지 24시간·7일·30일 필터와 탭 복귀 시 만료 대회 갱신
- 캘린더 알림 없음·1시간·1일·3일 전 선택, 실제 대회 기간 내보내기와 가져오기 안내
- 관심 대회·주최기관 JSON 백업, 검증 미리보기와 기존 목록 합치기
...
- 개별 출처의 잘못된 레코드 격리, DACON·Devpost·itch 일부 상세 요청 실패의 상태 표시
- 최신 대회가 운영 상세 URL에서 404가 되던 문제: Worker 사이트맵·SEO도 최신 검증 데이터를 사용
- 한국 시간 기준 D-day와 날짜 표시, 원격 데이터의 상세 선택 필드 검증
- 브라우저 저장소 접근이 차단되어도 화면과 데이터 요청이 유지되도록 처리
- 외부 제목에 포함된 특수 문자열의 SEO 치환 오류
