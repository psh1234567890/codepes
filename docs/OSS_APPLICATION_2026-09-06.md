...
- 이번 실제 수집: 2026-09-06 14:13 KST, 56개 대회. PS 18개, 보안 30개, AI·데이터 7개, 해커톤 1개, 게임 0개.
- 등록 출처 23곳 = 자동 수집 10곳 + 공고 감시 11곳 + 수동 검증 2곳. 이번 게시 항목을 제공한 출처는 5곳입니다. **23곳 모두가 현재 대회를 제공하는 것은 아닙니다.**
- 호남권·Baekjoon 감시 출처는 확인 실패. DACON·DAKER 수동 검증은 오래된 상태입니다. 현재 국내 대회와 게임잼 게시 항목은 0개입니다. 모든 대회를 빠짐없이 수집한다고 주장하지 않습니다.
...
2. 출처별 수집 결과를 합치기 전에 검증하여 한 출처의 잘못된 레코드가 전체 갱신을 중단시키지 않도록 격리.
3. Devpost·itch 상세 페이지의 일부 확인 실패도 출처 상태에서 구분. 현재는 이전 항목을 유지한 부분 실패가 전체 성공 상태에 가려질 수 있음.
